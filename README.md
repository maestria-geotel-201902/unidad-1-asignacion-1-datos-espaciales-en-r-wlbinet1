...
## Reading layer `PROVCenso2010' from data source `/home/lbine/unidad-1-asignacion-1-datos-espaciales-en-r-wlbinet1/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '30', 'PROV'])
```
...
## geometry type:  MULTIPOLYGON
## dimension:      XY
## bbox:           xmin: 434288.4 ymin: 2048516 xmax: 487139.2 ymax: 2112302
## epsg (SRID):    32619
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
##    PROV REG  TOPONIMIA ENLACE                           geom
## 30   30  09 HATO MAYOR   0930 MULTIPOLYGON (((480565.4 21...
plot(miprov)
```
...
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
##   PROV REG  TOPONIMIA ENLACE                           geom
## 1   30  09 HATO MAYOR   0930 MULTIPOLYGON (((480565.4 21...
##           areaenm2
## 1 1317574849 [m^2]
prov %>% dplyr::filter(PROV %in% '30') %>% dplyr::select(areaenm2) %>% plot
```
...
``` r
prov %>% dplyr::filter(PROV %in% c('30', '12', '25')) %>% dplyr::select(areaenm2) %>% plot
```
...
## Reading layer `MUNCenso2010' from data source `/home/lbine/unidad-1-asignacion-1-datos-espaciales-en-r-wlbinet1/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '30')
mimun %>% plot
```
